// THIS SOLUTION WORKING ONLY WITH  NodeJS 11 and above!
const snail = array => {
  
  const middlePart = array.slice(1, array.length - 1).map(row => row.slice(1, row.length - 1))

  return [
    // return first array
    array[0],
    // return last element of each row
    array.slice(1, array.length - 1).map(row => row[row.length - 1]),
    // if array nested return last array in reverse
    array.length > 1 ? array[array.length - 1].reverse() : [],
    // return first elem of each row in reverse
    array.slice(1, array.length - 1).reverse().map(row => row[0]),
    // iterating througth middlePart
    middlePart.length > 0 ? snail(middlePart) : [] 
  ].flat()
}
